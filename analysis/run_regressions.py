...
from utils.annotator import CurveAnnotator, get_features_dataframe
...
def run_lrts(dataframe, target_var):
    ...
    print('    LRT significance: p={}'.format(p_value))
    # POS.
    pos_reg = smf.ols(formula='{} ~ unigram + ngram + context + pos + 1'.format(target_var), data=dataframe).fit()
    likelihood_ratio = -2.0 * (context_reg.llf - pos_reg.llf)
    df = pos_reg.df_model - context_reg.df_model
    p_value = scipy.stats.chi2.sf(likelihood_ratio, df)
    print('  Unigram+ngram+context+POS R^2: {}'.format(pos_reg.rsquared_adj))
    print('    LRT significance: p={}'.format(p_value))
    print('    Improvement (excluding interactions): +{}'.format(pos_reg.rsquared_adj - context_reg.rsquared_adj))
    # print('')
...
def main():
    ...
    for target_var in ['surprisal', 'var_steps', 'aoa', 'forgettability']:
        run_lrts(dataframe, target_var)
